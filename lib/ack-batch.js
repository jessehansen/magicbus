...
class AckBatch {
  ...
  addMessage(message) {
    ...
    this.logger.debug(
      `New pending tag ${status.tag} on queue ${this.name} - ${this.connectionName}`
    );
  }
   ...
   */
  lastByStatus(status) {
    return findLast(this.messages, (x) => x.status === status);
  }
  ...
  resolveTag(tag, operation, inclusive) {
    ...
    this.logger.debug(
      `${operation} ${removed.length} tags (${
        inclusive ? "inclusive" : "individual"
      }) on ${this.name} - ${this.connectionName}. (Next ack: ${
        this.firstAck || 0
      }, Next nack: ${this.firstNack || 0}, Next reject: ${
        this.firstReject || 0
      })`
    );
  ...
  ackOrNackSequence() {
    try {
      ...
      if (firstStatus === "pending") {
        this.logger.silly(
          "First message in batch pending, cannot process batch"
        );
        return;
      }
      ...
      this.logger.error(
        `An exception occurred while trying to resolve ack/nack sequence '
        + 'on ${this.name} - ${this.connectionName}`,
        error
      );
    }
  }
  ...
  processBatch() {
    ...
    if (!this.acking) {
      ...
      let hasPending =
        this.messages.findIndex((x) => x.status === "pending") >= 0;
      let hasAck = this.firstAck;
      let hasNack = this.firstNack;
      let hasReject = this.firstReject;

      this.logger.silly(
        `Pending: ${hasPending}, Ack: ${hasAck}, Nack: ${hasNack}, Reject: ${hasReject}`
      );
  ...
  resolveAll(status, first, last) {
    ...
    if (this.messages.length !== 0) {
      ...
      this.logger.debug(
        `${status} ALL (${this.messages.length}) tags on ${this.name} - ${this.connectionName}.`
      );
      this.resolver(status, { tag: lastTag, inclusive: true }).then(() => {
        this[last] = lastTag;
  ...
  flush() {
    this.logger.debug(
      `Flushing ack batch with ${this.messages.length} messages`
    );
    const step = () => {
      let firstMessage = this.messages[0];
      if (firstMessage === undefined) {
      ...
      if (firstMessage.status === "pending") {
        this.logger.warn(
          "!!! Cannot flush ack batch because some messages are pending !!!"
        );
        return Promise.resolve();
      }
  ...
  getMessageOps(tag) {
    ...
    return {
      ...
      ack: () => {
        this.logger.silly(
          `Marking tag ${tag} as ack'd on queue ${this.name} - ${this.connectionName}`
        );
      ...
      nack: () => {
        this.logger.silly(
          `Marking tag ${tag} as nack'd on queue ${this.name} - ${this.connectionName}`
        );
      ...
      reject: () => {
        this.logger.silly(
          `Marking tag ${tag} as rejected on queue ${this.name} - ${this.connectionName}`
        );
