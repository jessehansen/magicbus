...
class AckBatch {
  ...
  removeByStatus (status) {
    this.messages = this.messages.filter((message) => message.status !== status)
  }
  ...
  ackOrNackSequence () {
    try {
      ...
      this.logger.error(`An exception occurred while trying to resolve ack/nack sequence '
        + 'on ${this.name} - ${this.connectionName}`, error)
    }
  }
  ...
  processBatch () {
    ...
    if (!this.acking) {
      ...
      let hasPending = (this.messages.findIndex((x) => x.status === 'pending') >= 0)
