...
function AckBatch(name, connectionName, resolver, logger) {
  this.name = name;
...
AckBatch.prototype._ackOrNackSequence = function() {
  var firstMessage, firstStatus, sequenceEnd, call, i;
  try {
    firstMessage = this.messages[0];
    if (firstMessage === undefined) {
      return;
    }
    firstStatus = firstMessage.status;
    ...
    for (i = 1; i < _.size(this.messages) - 1; i++) {
...
AckBatch.prototype._resolveAll = function(status, first, last) {
  ...
  if (this.messages.length !== 0) {
    lastTag = this._lastByStatus(status).tag;
...
AckBatch.prototype.flush = function() {
  ...
  function step() {
    var firstMessage = self.messages[0];
    if (firstMessage === undefined) {
      return Promise.resolve();
    }
    if (firstMessage.status === 'pending') {
      self.logger.warn('!!! Cannot flush ack batch because some messages are pending !!!');
      return Promise.resolve();
    }
    ...
    return Promise.delay(0) // see comment about heisenbug above. This gives amqplib enough time to actually resolve all messages
      .then(function(){
        ...
        self.logger.debug('Ack batch is empty');
        return Promise.resolve();
      });
  };
  return step();
