...
AckBatch.prototype._ackOrNackSequence = function() {
  try {
    ...
    this.logger.error(util.format('An exception occurred while trying to resolve ack/nack sequence on %s - %s', this.name, this.connectionName), err);
  }
};
...
AckBatch.prototype._resolveAll = function(status, first, last) {
  ...
  if (this.messages.length !== 0) {
    ...
    this.logger.info('%s ALL (%d) tags on %s - %s.',
      status,
    ...
    this.resolver(status, { tag: lastTag, inclusive: true })
      .then(function() {
        ...
        if (count > 0 && this.messages.length === 0) {
          this.logger.info('No pending tags remaining on queue %s - %s', this.name, this.connectionName);
...
AckBatch.prototype._resolveTag = function(tag, operation, inclusive) {
  ...
  this.logger.info('%s %d tags (%s) on %s - %s. (Next ack: %d, Next nack: %d, Next reject: %d)',
    operation,
    removed.length,
...
AckBatch.prototype.addMessage = function(message) {
  ...
  this.logger.debug('New pending tag %d on queue %s - %s', status.tag, this.name, this.connectionName);
};
...
AckBatch.prototype.getMessageOps = function(tag) {
  ...
  return {
    message: message,
    ack: function() {
      this.logger.debug('Marking tag %d as ack\'d on queue %s - %s', tag, this.name, this.connectionName);
      this.firstAck = this.firstAck || tag;
      message.status = 'ack';
    }.bind(this),
    nack: function() {
      this.logger.debug('Marking tag %d as nack\'d on queue %s - %s', tag, this.name, this.connectionName);
      this.firstNack = this.firstNack || tag;
      message.status = 'nack';
    }.bind(this),
    reject: function() {
      this.logger.debug('Marking tag %d as rejected on queue %s - %s', tag, this.name, this.connectionName);
