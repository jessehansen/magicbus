...
function Publisher(
  ...
  const getPublishOptions = (
    msg,
    perCallPublishOptions,
    envelopePublishOptions
  ) =>
    Object.assign(
      {},
  ...
  const doPublish = (message, kind, options = {}) => {
    ...
    return pipeline
      .prepare()(msg, options)
      .then(() => {
        let messageParameters = getPublishOptions(
          msg,
          options.publishOptions,
          envelope.getPublishOptions(message, kind, serializer)
        );
  ...
  const publish = (eventName, data, options) => {
    typeof eventName === "string" || assert.fail("eventName must be a string");
  ...
  const send = (message, messageType, options) => {
    message || assert.fail("message must be provided");
    !messageType ||
