...
function Publisher (broker, envelope, pipeline, routeName, routePattern, logger) {
  ...
  const doPublish = (message, kind, options) => {
    ...
    return pipeline.prepare()(msg, options).then(() => {
      let messageParameters = getPublishOptions(msg, (options || {}).publishOptions)
      messageParameters.routingKey = envelope.getRoutingKey(message, kind)
      messageParameters.payload = envelope.serialize(msg.payload)
