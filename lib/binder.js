...
Binder.prototype.assertTopology = function Binder$assertTopology(route) {
  this._logger.debug(`Asserting topology for route ${route.name}`);
  return route.pattern.assertTopology(this._topology, route.serviceDomainName, route.appName, route.name);
};
