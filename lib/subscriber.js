...
function Subscriber(consumer, eventDispatcher, logger, events) {
   ...
   */
  const internalHandler = (data, messageTypes, msg, actions) => {
    logger.debug(
      "Subscriber received message with types " +
        JSON.stringify(messageTypes) +
        ", handing off to event dispatcher."
    );
    return eventDispatcher
      .dispatch(messageTypes, data, msg, actions)
      .catch((error) => {
  ...
  const startSubscription = (options) =>
    consumer.startConsuming(internalHandler, options);
