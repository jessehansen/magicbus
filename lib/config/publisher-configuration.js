const PublisherTopology = require('../publisher/topology')
...
const publisherConfigurator = (broker) => {
  ...
  let outputFilters = [() => envelopeFactory(), () => serializerFactory()]
  ...
  const useMagicEnvelope = () => {
    envelopeFactory = MagicEnvelope
  }
  ...
  const overrideFilters = ({ input = false, output = false }) => {
    if (input !== false) {
      inputFilters = Array.from(input)
    }
    if (output !== false) {
      outputFilters = Array.from(output)
    }
  }
