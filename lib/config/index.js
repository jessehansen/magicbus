...
const Configurator = (logger, events) => {
  const getParams = (configFunc, configurator) => {
    ...
    if (configurator) {
      let configFunctions = Object.assign(
        {},
        config.getConfigurator(),
        loggerConfig.getConfigurator()
      );
      let configTarget = {};
  ...
  function createTopology(connectionInfo, logger) {
    ...
    if (typeof connectionInfo === "string") {
      ...
      options = {
        ...
        user:
          parsed.auth && parsed.auth.indexOf(":") !== -1
            ? unescape(parsed.auth.split(":")[0])
            : "guest",
        pass:
          parsed.auth && parsed.auth.indexOf(":") !== -1
            ? unescape(parsed.auth.split(":")[1])
            : "guest",
    ...
    let connection = connectionFactory(
      options,
      logger.withNamespace("connection")
    );
    let topology = topologyFactory(
      connection,
      logger.withNamespace("topology")
    );
    return topology;
  }
  ...
  function createBroker(
    ...
    return brokerFactory(
      serviceDomainName,
      appName,
      createTopology(connectionInfo, params.logger),
      params.logger
    );
  }
  ...
  function createSubscriber(broker, configurator) {
    ...
    if (typeof broker === "function") {
      // assume it's a configurator function that uses a custom consumer
      configurator = broker;
      broker = null;
    }
    params = getParams(subscriberConfigurator, configurator);
    if (!params.consumer) {
      params.consumer = consumerFactory(
        broker,
        params.envelope,
        params.serializer,
        params.pipeline,
