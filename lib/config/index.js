...
const Configurator = (logger, events) => {
  const getParams = (configFunc, configurator, broker) => {
    let config = configFunc(broker)
  ...
  function createPublisher (broker, configurator) {
    let params = { broker, ...getParams(publisherConfigurator, configurator, broker) }
    return publisherFactory(params)
  };
  ...
  function createConsumer (broker, configurator) {
    let params = { broker, events, ...getParams(consumerConfigurator, configurator, broker) }
    return consumerFactory(params)
  };
