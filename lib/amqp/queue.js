...
module.exports = function(options, topology, log) {
  ...
  function drain(channel, options, connectionName) {
    qLog.debug(`Draining queue ${options.name} - ${connectionName}`);
    return channel.purgeQueue(options.name).then(function(){
      qLog.debug(`Successfully drained queue ${options.name} - ${connectionName}`);
    });
  }
  ...
  function destroy(channel, options, messages, released) {
    ...
    function finalize() {
      if (messages.messages.length) {
        qLog.warn(`!!! Finalizing queue ${options.name} - ${topology.connection.name} with ${messages.messages.length} messages pending !!!`)
      }
  ...
  return {
    ...
    drain: drain.bind(undefined, channel, options, topology.connection.name)
  };
};
