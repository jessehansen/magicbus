...
module.exports = function(options, topology, log) {
  var channel, messages, subscriber;
  ...
  channel = getChannel(topology.connection);
  messages = new AckBatch(options.name, topology.connection.name, resolveTags(channel, options.name, topology.connection.name), log);
  subscriber = subscribe.bind(undefined, options.name, channel, topology, messages, options);
