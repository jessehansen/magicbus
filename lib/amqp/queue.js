...
module.exports = function(options, topology, log) {
  var qLog = log.scope('queue');
  var topLog = log.scope('topology');
  var unhandledLog = log.scope('unhandled');
  ...
  function getReply(channel, raw, replyQueue, connectionName) {
    ...
    return function(reply, more, replyType) {
      ...
      if (replyTo) {
        ...
        qLog.debug('Replying to message %s on %s - %s with type %s',
  ...
  function getUntrackedOps(channel, raw, messages) {
    ...
    return {
      ack: noOp,
      nack: function() {
        qLog.debug('Nacking tag %d on %s - %s', raw.fields.deliveryTag, messages.name, messages.connectionName);
        channel.nack({ fields: { deliveryTag: raw.fields.deliveryTag } }, false);
      },
      reject: function() {
        qLog.debug('Rejecting tag %d on %s - %s', raw.fields.deliveryTag, messages.name, messages.connectionName);
  ...
  function getNoBatchOps(channel, raw, messages, noAck) {
    ...
    if (noAck) {
      ...
      ack = function() {
        qLog.debug('Acking tag %d on %s - %s', raw.fields.deliveryTag, messages.name, messages.connectionName);
  ...
  function resolveTags(channel, queue, connection) {
    return function(op, data) {
      switch (op) {
        case 'ack':
          qLog.debug('Acking tag %d on %s - %s', data.tag, queue, connection);
          return channel.ack({ fields: { deliveryTag: data.tag } }, data.inclusive);
        case 'nack':
          qLog.debug('Nacking tag %d on %s - %s', data.tag, queue, connection);
          return channel.nack({ fields: { deliveryTag: data.tag } }, data.inclusive);
        case 'reject':
          qLog.debug('Rejecting tag %d on %s - %s', data.tag, queue, connection);
  ...
  function subscribe(channelName, channel, topology, messages, options) {
    ...
    qLog.info('Starting subscription %s - %s', channelName, topology.connection.name);
  ...
  function unsubscribe(channel, options) {
    if (channel.tag) {
      qLog.info('Unsubscribing from queue %s with tag %s', options.name, channel.tag);
