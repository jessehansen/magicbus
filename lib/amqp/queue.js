...
const queueFactory = (options, topology, log) => {
  ...
  const resolver = (op, data) => {
    switch (op) {
      case "ack":
        qLog.debug(
          `Acking tag ${data.tag} on ${channelName} - ${connectionName}`
        );
        return channel.ack(
          { fields: { deliveryTag: data.tag } },
          data.inclusive
        );
      case "nack":
        qLog.debug(
          `Nacking tag ${data.tag} on ${channelName} - ${connectionName}`
        );
        return channel.nack(
          { fields: { deliveryTag: data.tag } },
          data.inclusive
      ...
      case "reject":
        qLog.debug(
          `Rejecting tag ${data.tag} on ${channelName} - ${connectionName}`
        );
        return channel.nack(
          { fields: { deliveryTag: data.tag } },
          data.inclusive,
  ...
  const purge = () => {
    ...
    return channel.purgeQueue(channelName).then(() => {
      qLog.debug(
        `Successfully purged queue ${channelName} - ${connectionName}`
      );
    });
  };
   ...
   * @public
   */
  const getMessageCount = () => (messages ? messages.messages.length : 0);
  ...
  const getNoBatchOps = (raw) => {
    ...
    return {
      ...
      nack: () => {
        ...
        channel.nack(
          { fields: { deliveryTag: raw.fields.deliveryTag } },
          false
        );
      },
      reject: () => {
  ...
  */
  const subscribe = (callback, subscribeOptions) => {
    let consumeOptions = filterConsumeOptions(
      Object.assign({}, options, subscribeOptions)
    );
    ...
    qLog.info(
      `Starting subscription ${channelName} - ${connectionName} with ${JSON.stringify(
        consumeOptions
      )}`
    );
    return (consumeOptions.limit !== false
      ? channel.prefetch(consumeOptions.limit || 500)
      : Promise.resolve()
    )
      .then(() => {
        const handler = (raw) => {
          ...
          qLog.silly(
            `Received message on queue ${channelName} - ${connectionName}`
          );
          ...
          try {
            ...
            qLog.error(
              `Error handing message on queue ${channelName} - ${connectionName}`,
              error
            );
            ops.reject();
          }
  ...
  const unsubscribe = () => {
    if (channel.tag) {
      qLog.info(
        `Unsubscribing from queue ${channelName} with tag ${channel.tag}`
      );
  ...
  const define = () => {
    if (options.check) {
      topLog.info(
        `Connecting to queue '${channelName}' on connection '${connectionName}'`
      );
      return channel.checkQueue(channelName);
    }
    let aqOptions = getAssertQueueOptions();
    topLog.info(
      `Declaring queue '${channelName}' on connection '${connectionName}'` +
  ...
  const destroy = (released) => {
    const flush = () =>
      messages.messages.length > 0 && !released
        ? messages.flush()
        : Promise.resolve();
    ...
    return flush()
      .then(() => unsubscribe(channel, options))
