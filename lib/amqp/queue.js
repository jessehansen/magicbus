...
const queueFactory = (options, topology, log) => {
  ...
  */
      ...
      .then(() => {
        const handler = (raw) => {
          ...
          try {
            ...
            qLog.error(`Error handing message on queue ${channelName} - ${connectionName}`, error)
            ops.reject()
          }
        }
  ...
  const destroy = (released) => {
    const flush = () => messages.messages.length > 0 && !released ? messages.flush() : Promise.resolve()
