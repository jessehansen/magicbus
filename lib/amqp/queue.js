...
module.exports = function(options, topology, log) {
   ...
   * @param {bool} subscribeOptions.noBatch - if true, ack/nack/reject operations will execure immediately and not be batched
  ...
  */
  function subscribe(callback, subscribeOptions) {
    ...
    qLog.info(`Starting subscription ${channelName} - ${connectionName} with ${JSON.stringify(_.omit(consumeOptions, 'name'))}`);
    return (consumeOptions.limit ? channel.prefetch(consumeOptions.limit) : Promise.resolve())
      .then(() => {
        return channel.consume(channelName, function(raw) {
          let ops = getResolutionOperations(raw);
