 ...
 * @param {Any} options.persistent - ignored here
 * @param {Any} options.publishTimeout - ignored here
...
module.exports = function Exchange(options, topology, publishLog, log) {
  const exLog = log.scoped('exchange');
  const topLog = log.scoped('topology');
  ...
  function define() {
    ...
    topLog.info(`Declaring ${options.type} exchange \'${options.name}\' on connection \'${connectionName}\' with the options: ${JSON.stringify(_.omit(libOptions, ['name', 'type']))}`);
  ...
  function publish(message) {
    ...
    exLog.debug(`Publishing message (type: ${publishOptions.type} topic: ${effectiveKey}, sequence: ${message.sequenceNo}, correlation: ${publishOptions.correlationId}, replyTo: ${publishOptions.replyTo}) to ${type} exchange ${channelName} - ${connectionName}`);

    return channel.publish(channelName, effectiveKey, payload, publishOptions);
  }
