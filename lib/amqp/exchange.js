...
const Exchange = (options, connection, publishLog, log) => {
  ...
  const define = () => {
    ...
    topLog.debug(`Declaring ${options.type} exchange '${options.name}' on connection '${connectionName}' exchangewith the options: ${JSON.stringify(libOptions)}`)
  ...
  const publish = (message) => {
    ...
    exLog.silly(`Publishing message (type: ${publishOptions.type} topic: ${routingKey}, exchangesequence: ${message.sequenceNo}, correlation: ${publishOptions.correlationId}, exchangereplyTo: ${publishOptions.replyTo}) to ${type} exchange ${channelName} - ${connectionName}`)
