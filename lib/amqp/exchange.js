 ...
 * @param {bool} options.check - if true, instead of assertExchange checkExchange is used and other options are ignored
 ...
 */
const createExchange = (options, topology, publishLog, log) => {
  ...
  const define = () => {
    if (options.check) {
      topLog.debug(
        `Connecting to ${options.type} exchange '${options.name}' on connection '${connectionName}'`
      );
      return channel.checkExchange(options.name);
    }
    let libOptions = getLibOptions({ alternate: "alternateExchange" }, [
      "persistent",
      "publishTimeout",
    ]);
    topLog.debug(
      `Declaring ${options.type} exchange '${options.name}' on connection '${connectionName}' ` +
  ...
  const publish = (message) => {
    ...
    let effectiveKey =
      message.routingKey === ""
        ? ""
        : message.routingKey || publishOptions.type;
    exLog.silly(
      `Publishing message (type: ${publishOptions.type} topic: ${effectiveKey}, ` +
    ...
    return channel
      .publish(channelName, effectiveKey, payload, publishOptions)
      .then((result) => {
        if (result === false) {
