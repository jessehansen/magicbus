...
module.exports = function(factory, target, release, _disposalEvent, logger) {
  var machine;
  var disposalEvent = _disposalEvent || 'close';
      ...
      function onAcquired(o) {
        ...
        if (this.item.on) {
          this.disposeHandle = this.item.once(disposalEvent, function(/* err */) {
            logger.info('Resource lost, releasing', arguments);
            this.emit('lost');
    ...
    states: {
      ...
      'acquired': {
        ...
        operate: function(call) {
          var result;
          try {
            result = this.item[call.operation].apply(this.item, call.argList);
