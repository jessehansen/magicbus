...
const machineFactory = (
  ...
  const PromiseMachine = machina.Fsm.extend({
    ...
    doAcquire: function () {
      ...
      const onAcquired = (o) => {
        ...
        this.waitInterval = 0;
        if (this.item.on) {
          this.disposeHandle = this.item.once(
            disposalEvent || "close",
            (err) => {
      ...
      factory().then(onAcquired, onAcquisitionError).catch(onException);
    },
    ...
    states: {
      ...
      acquired: {
        ...
        operate: function (call) {
          try {
            let result = this.item[call.operation].apply(
              this.item,
              call.argList
            );
            if (result && result.then) {
              result.then(call.resolve).then(null, call.reject);
            } else {
              call.resolve(result);
            }
          } catch (error) {
  ...
  machine.on("transition", (data) => {
    logger.debug(
      `Machine ${machine.name || "unnamed"}: ${data.fromState} -> ${
        data.toState
      }`
    );
  });
  machine.on("deferred", (data) => {
    logger.silly(
      `Machine ${machine.name || "unnamed"}: deferred method ${JSON.stringify(
        data
      )}`
    );
  });
