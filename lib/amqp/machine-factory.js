 ...
 */
module.exports = function CreatePromiseMachine(factory, target, release, disposalEvent, disableDeferForOperations, logger) {
  ...
  machine.on('transition', (data) => {
    logger.debug(`Machine ${machine.name || logger.scope}: ${data.fromState} -> ${data.toState}`);
  });
  machine.on('deferred', (data) => {
    logger.debug(`Machine ${machine.name || logger.scope}: deferred method ${JSON.stringify(data)}`);
  });
  return machine;
};
