...
module.exports = function(factory, target, release, disposalEvent, logger) {
  ...
  var PromiseMachine = machina.Fsm.extend({
    ...
    _acquire: function() {
      ...
      function onAcquisitionError(err) {
        logger.debug('Resource acquisition failed with \'%s\'', err);
      ...
      function onException(ex) {
        logger.debug('Resource acquisition failed with \'%s\'', ex);
        this.emit('failed', ex);
        this.handle('failed');
      }
      factory()
