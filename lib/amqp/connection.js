...
function CreateRoundRobinConnectFunction(parameters, logger) {
  ...
  const getOptions = (p) => {
    ...
    if (p.caPaths) {
      ...
      options.ca = _.map(list, function(caPath) {
  ...
  const getNextUri = function() {
    var server = getNext(p.servers);
    var port = getNext(p.ports);
    var uri = getUri(p.protocol, p.user, escape(p.pass), server, port, p.vhost, p.heartbeat);
    return uri;
  };
  ...
  const connect = function() {
    return new Promise((resolve, reject) => {
      let attempted = [];
      const attempt = () => {
        var nextUri = getNextUri();
        logger.info(`Attempting connection to ${p.name} (${nextUri})`);
        const onConnection = (connection) => {
          connection.uri = nextUri;
          logger.info(`Connected to ${p.name} (${nextUri})`);
          resolve(connection);
        };
        const onConnectionError = (err) => {
          logger.info(`Failed to connect to ${p.name} (${nextUri})`, err);
        ...
        if (_.indexOf(attempted, nextUri) < 0) {
          ...
          logger.info(`Cannot connect to ${p.name} - all endpoints failed`);
 ...
 * @param {String} options.port - list of ports to connect to, separated by ','. Must have the same number of entries as options.server
...
module.exports = function CreateAmqpConnectionMachine(options, logger) {
  ...
  let log = logger.scoped('amqp-connection');
  var connect = CreateRoundRobinConnectFunction(options, log);
  return MachineFactory(connect, AmqpConnection, close, 'close', [], log);
};
