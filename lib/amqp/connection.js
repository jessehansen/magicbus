...
var Adapter = function(parameters, log) {
  var logger = log.scope('amqp-connection');
...
Adapter.prototype.connect = function() {
  return new Promise(function(resolve, reject) {
    ...
    attempt = function() {
      ...
      this.log.info('Attempting connection to %s (%s)', this.name, nextUri);
      function onConnection(connection) {
        connection.uri = nextUri;
        this.log.info('Connected to %s (%s)', this.name, nextUri);
        resolve(connection);
      }
      function onConnectionError(err) {
        this.log.info('Failed to connect to %s (%s) with', this.name, nextUri, err);
      ...
      if (_.indexOf(attempted, nextUri) < 0) {
        ...
        this.log.info('Cannot connect to %s - all endpoints failed', this.name);
