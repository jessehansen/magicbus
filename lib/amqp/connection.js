...
function CreateRoundRobinConnectFunction(parameters, log) {
  ...
  let logger = log.scoped('amqp-connection');
...
module.exports = function CreateAmqpConnectionMachine(options, logger) {
  ...
  var connect = CreateRoundRobinConnectFunction(options, logger);
  return MachineFactory(connect, AmqpConnection, close, 'close', logger);
};
