...
const createRoundRobinConnect = (parameters = {}, log) => {
  const getConnectionParameters = () => {
    ...
    p.useSSL =
      p.certPath || p.keyPath || p.passphrase || p.caPaths || p.pfxPath;
    if (p.useSSL) {
      p.protocol = "amqps://";
    }
    return p;
  ...
  const getOptions = async (p) => {
    ...
    if (p.caPaths) {
      options.ca = await Promise.all(
        splitAndTrim(p.caPaths).map((caPath) => readFileAsync(caPath))
      );
    }
    return options;
  };
  ...
  let limit =
    p.servers.length > p.ports.length ? p.servers.length : p.ports.length;

  const getNext = (list) =>
    connectionIndex < list.length ? list[connectionIndex] : list[0];
