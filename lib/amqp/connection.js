...
const getUri = (protocol, user, pass, server, port, vhost, heartbeat) =>
  `${protocol}${escape(user)}:${escape(pass)}@${escape(server)}:${Math.floor(port)}/${escape(vhost)}?heartbeat=${Math.floor(heartbeat)}`
...
const getOptions = async (p) => {
  let options = { noDelay: true }
  if (p.timeout) {
    options.timeout = p.timeout
  }
  if (p.certPath) {
    options.cert = await readFileAsync(p.certPath)
  }
  if (p.keyPath) {
    options.key = await readFileAsync(p.keyPath)
  }
  if (p.passphrase) {
    options.passphrase = p.passphrase
  }
  if (p.pfxPath) {
    options.pfx = await readFileAsync(p.pfxPath)
  }
  if (p.caPaths) {
    options.ca = await Promise.all(splitAndTrim(p.caPaths).map((caPath) => readFileAsync(caPath)))
  }
  return options
}
...
const createRoundRobinConnect = (parameters = {}, log) => {
  ...
  let optionPromise = getOptions(p)

  let connectionIndex = 0
  let limit = Math.max(p.servers.length, p.ports.length)
  ...
  const getNextUri = () => {
    ...
    let uri = getUri(p.protocol, p.user, p.pass, server, port, p.vhost, p.heartbeat)
    return uri
  }
  ...
  const connect = () => new Promise((resolve, reject) => {
    ...
    const attempt = () => {
      ...
      if (attempted.indexOf(nextUri) < 0) {
        optionPromise
          .then((options) => amqp.connect(nextUri, options))
...
const AmqpConnectionMachine = (options, logger) => {
...
module.exports = AmqpConnectionMachine
