...
/**
 * Represents a log of published messages in sequential order. Used for keeping messages that need to be (re)published upon exchange (re)connection
 */
function PublishLog() {
  let state = {
    count: 0,
   ...
   */
  const reset = () => {
    let list = _.map(state.messages, function(m) {
      delete m.sequenceNo;
      return m;
    });
