...
Object.defineProperties(Broker.prototype, {
  ...
  shutdown: {
    value: function() {
      ...
      if (this._ackInterval) {
        clearInterval(this._ackInterval);
        this._ackInterval = null;
      }
      if (self._topology) {
        return self._topology.connection.close(true).then(function(){
          self._topology = null;
        });
      }
      return Promise.resolve();
  ...
  connect: {
    value: function() {
      ...
      if (!this._topology) {
        ...
        topology = Topology(connection, this._connectionInfo || {}, unhandledStrategies, this._logger);
        ...
        this._ackInterval = setInterval(function() {
          signal.publish('ack');
        }, 500);
      }
  ...
  _getExchange: {
    value: function(routeName) {
      var self = this;
      self.connect();
      return self._createRouteTopology(routeName)
        .then(function(route){
  ...
  _createRouteTopology: {
    value: function(routeName) {
      ...
      return route.asserting = route.pattern.createTopology(self._topology, self._serviceDomainName, self._appName, routeName)
   ...
   */
  bind: {
    value: function (publishingRouteName, consumingRouteName, options) {
      var exchangeName, queueName;
      var self = this;

      self.connect();

      return self._createRouteTopology(publishingRouteName)
      .then(function() {
        exchangeName = self._routes[publishingRouteName].exchangeName;
        return self._createRouteTopology(consumingRouteName);
      }).then(function() {
        queueName = self._routes[consumingRouteName].queueName;
