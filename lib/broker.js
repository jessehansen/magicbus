...
function Broker (serviceDomainName, appName, topology, logger) {
  ...
  const createRouteTopology = (routeName) => {
    ...
    assert.func(route.pattern)
    route.asserting = route.pattern(topology, serviceDomainName, appName, routeName)
   ...
   */
  const publish = (routeName, message) => getExchange(routeName).then(function (exchange) {
   ...
   */
  const consume = (routeName, callback, options) => getQueue(routeName).then(function (queue) {
   ...
   */
  const purgeRouteQueue = (routeName) => getQueue(routeName).then(function (queue) {
    return queue.purge()
  })
