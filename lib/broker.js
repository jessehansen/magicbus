...
const createBroker = (serviceDomainName, appName, topology, logger) => {
   ...
   */
  const getRoutePattern = (routeName) => routes[routeName].pattern;
  ...
  const createRouteTopology = (routeName) => {
    ...
    route.asserting = route
      .pattern(topology, serviceDomainName, appName, routeName)
      .then((topologyNames) => {
      ...
      .catch((error) => {
        logger.error(`Failed to assert route ${routeName}`, error);
  ...
  const getExchange = (routeName) => {
    activate();
    return createRouteTopology(routeName).then((route) => {
  ...
  const publish = (routeName, message) =>
    getExchange(routeName).then((exchange) => {
      logger.debug(
        `Publishing message to exchange ${exchange.name} with routing key ${message.routingKey}`
      );
  ...
  const purgeRouteQueue = (routeName) =>
    getQueue(routeName).then((queue) => queue.purge());
