          ...
          self._createConnection().then(function(conn) {
            conn.createChannel().then(function(channel) {
              ...
              switch (self._routes[routeName].pattern) {
                case 'worker':
                  var workerQueueName = self._serviceDomainName + '.' + self._appName + '.' + routeName;
                ...
                case 'listener':
                  var listenerExchangeName = self._serviceDomainName + '.' + self._appName + '.' + routeName;
                ...
                default:
                  self._routes[routeName].pattern.assertRoute(self._serviceDomainName, self._appName, routeName, channel).then(function(topologyNames){
