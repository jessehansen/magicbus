...
class Topology {
  ...
  configureBindings (bindings, list) {
    ...
    let promises = _.map((x) => actualDefinitions(x), (def) => this.createBinding(
      {
  ...
  configureQueues (queues, list) {
    ...
    let promises = _.map((x) => actualDefinitions(x), (def) => this.createQueue(def))
    return Promise.all(promises)
  }
  ...
  configureExchanges (exchanges, list) {
    ...
    let promises = _.map((x) => actualDefinitions(x), (def) => this.createExchange(def))
    return Promise.all(promises)
  }
  ...
  onReconnect () {
    ...
    this.promises = {}
    let prerequisites = _.map(this.channels, (channel) => channel.check ? channel.check() : Promise.resolve(true))
