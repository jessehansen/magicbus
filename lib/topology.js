...
class Topology {
  ...
  configureBindings(bindings, list) {
    ...
    let promises = actualDefinitions.map((def) =>
      this.createBinding({
  ...
  createBinding(options) {
    ...
    if (!promise) {
      ...
      this.logger.verbose(
        `Binding '${target}' to '${source}' on '${this.connection.name}'` +
          ` with keys: ${JSON.stringify(keys)}`
      );
      this.promises[id] = promise = Promise.all(
  ...
  createPrimitive(Primitive, primitiveType, options) {
    let errorFn = (err) =>
      new Error(
        `Failed to create ${primitiveType} '${options.name}' on connection ` +
          `'${this.connection.name}' with '${err ? err.message || err : "N/A"}'`
      );
    let definitions =
      primitiveType === "exchange"
        ? this.definitions.exchanges
        : this.definitions.queues;
    ...
    if (!promise) {
      this.promises[channelName] = promise = new Promise((resolve, reject) => {
        ...
        let primitive = (this.channels[channelName] = Primitive(
          options,
          this.connection,
          this,
          this.logger
  ...
  /**
   * Connects to an existing exchange. Adds a channel exchange:[name]
   *
   * @public
   * @param {String} name - exchange name
   */
  connectExchange(name) {
    return this.createPrimitive(ExchangeMachine, "exchange", {
      name,
      check: true,
    });
  }
  ...
  /**
   * Connects to an existing queue. Adds a channel queue:[name]
   *
   * @public
  ...
  deleteQueue(name) {
    ...
    if (channel) {
      ...
      delete this.channels[key];
      this.logger.verbose(
        `Deleting queue '${name}' on connection '${this.connection.name}'`
      );
    }
    let control = this.getChannel("control");
  ...
  onReconnect() {
    this.logger.verbose(
      `Reconnection to '${this.connection.name}' established - rebuilding topology`
    );
    this.promises = {};
    let prerequisites = Object.values(this.channels).map((channel) =>
      channel.check ? channel.check() : Promise.resolve(true)
    );
    Promise.all(prerequisites).then(() => {
      this.configureBindings(this.definitions.bindings, true).then(() => {
        this.logger.verbose(
          `Topology rebuilt for connection '${this.connection.name}'`
        );
 ...
 */
module.exports = function CreateTopology(
  connection,
  log,
  exchangeMachine,
  queueMachine
