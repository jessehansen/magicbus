      ...
      try {
        ...
        if (handlerResult && handlerResult.then) {
          handlerResult.then(function() {
            ...
            logger.error('Asynchronous handler failed.', err);
            actions.reject();
          });
        ...
        logger.error('Synchronous handler failed with error.', err);
        actions.reject();
      }
    }).prepare(function(eventSink) {
      eventSink.on('ack', function() {
        logger.info('Middleware acked message');
        messageHandled = true;
        ops.ack();
      });
      eventSink.on('nack', function() {
        logger.info('Middleware nacked message');
        messageHandled = true;
        ops.nack();
      });

      eventSink.on('reject', function() {
        logger.info('Middleware rejected message');
        messageHandled = true;
        ops.reject();
      });
      eventSink.on('error', function(err) {
        logger.warn('Middleware raised error', err);
        messageHandled = true;
        ops.reject();
      });
