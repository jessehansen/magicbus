...
Object.defineProperties(Consumer.prototype, {
        ...
        try {
          ...
          if (handlerResult && handlerResult.then) {
            handlerResult.then(function() {
              ...
              self._logger.warn('Asynchronous handler failed.', err);
              actions.reject();
            });
          ...
          self._logger.warn('Synchronous handler failed with error.', err);
          actions.reject();
        }
  ...
  /**
   * Retrieves an open channel for this consumer
   *
   * @public
   * @method
