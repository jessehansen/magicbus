...
const createConsumer = (
    ...
    pipeline
      ...
      .use((msg, actions) => {
        ...
        try {
          ...
          if (handlerResult && handlerResult.then) {
            handlerResult
              ...
              .catch((err) => {
                ...
                logger.error(
                  "Asynchronous handler failed with error, rejecting message.",
                  err
                );
                actions.reject();
              });
          ...
          logger.error(
            "Synchronous handler failed with error, rejecting message.",
            err
          );
          actions.reject();
        }
  ...
  const startConsuming = (messageCallback, options) => {
    if (consuming) {
      logger.error(
        "Attempted to start consuming on a consumer that was already active"
      );
