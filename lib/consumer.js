...
Object.defineProperties(Consumer.prototype, {
  ...
  _consumeCallback: {
    value: function(originalMessage, ops) {
      ...
      this._pipeline.clone().use(function(msg, actions) {
        ...
        data = self._getData(msg);
        messageTypes = self._getMessageTypes(msg);
        ...
        try {
          ...
          handlerResult = self._handler(data, messageTypes, msg, actions);
  ...
  purgeQueue: {
    value: function() {
      return this._broker.purgeRouteQueue(this._routeName);
    },
    enumerable: true
  },
