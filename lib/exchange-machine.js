...
var Channel = function(options, connection, topology, channelFn, logger) {
  ...
  var Fsm = machina.Fsm.extend({
    ...
    destroy: function() {
      logger.debug('Destroy called on exchange %s - %s (%d messages pending)', this.name, connection.name, this.published.count());
      var deferred = DeferredPromise();
      ...
      'destroyed': {
        _onEnter: function() {
          if (this.published.count() > 0) {
            logger.warn('%s exchange %s - %s was destroyed with %d messages unconfirmed',
              this.type,
              this.name,
              connection.name,
      ...
      'reconnected': {
        ...
        'bindings-completed': function() {
          ...
          var onRepublishFailed = function(err) {
            logger.error('Failed to republish %d messages on %s exchange, %s - %s with: %s',
              this.published.count(),
              this.type,
              this.name,
              connection.name,
