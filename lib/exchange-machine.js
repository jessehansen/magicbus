...
module.exports = function CreateExchangeMachine (options, connection, topology, logger, exchangeFactory) {
  ...
  const ExchangeMachine = machina.Fsm.extend({
    ...
    destroy: function () {
      let deferred = DeferredPromise()
      logger.debug(`Destroy called on exchange ${this.name} - ${connection.name} ` +
    ...
    republish: function () {
      ...
      if (undelivered.length > 0) {
        return Promise.map(undelivered, (message) => this.channel.publish(message))
      }
      ...
      destroyed: {
        _onEnter: function () {
          if (this.published.count() > 0) {
            logger.warn(`${this.type} exchange ${this.name} - ${connection.name} was destroyed ` +
      ...
      reconnected: {
        ...
        'bindings-completed': function () {
          ...
          const onRepublishFailed = (err) => {
            logger.error(`Failed to republish ${this.published.count()} messages on ${this.type} exchange, ` +
              `${this.name} - ${connection.name}`, err)
