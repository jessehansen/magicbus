 ...
 * @param {String} options.alternateExchange - an exchange to send messages to if this exchange can't route them to
      any queues. Alias: options.alternate
 ...
 * @param {Function} exchangeFactory - factory function that takes the options, topology, publish log, and the logger
      to create an Exchange
 * @returns {ExchangeMachine}
 */
module.exports = function CreateExchangeMachine(
  options,
  connection,
  topology,
  logger,
  ...
  const ExchangeMachine = machina.Fsm.extend({
    ...
    define: function (stateOnDefined) {
      ...
      this.channel.define().then(onDefined, onDefinitionError);
    },
    ...
    listenForConnectionEvents: function () {
      this.handlers.push(
        topology.on("bindings-completed", () => {
    ...
    destroy: function () {
      let deferred = DeferredPromise();
      logger.debug(
        `Destroy called on exchange ${this.name} - ${connection.name} ` +
          `(${this.published.count()} messages pending)`
      );
    ...
    publish: function (message) {
      let publishTimeout =
        ...
        message.connectionPublishTimeout ||
        0;
      ...
      return new Promise((resolve, reject) => {
        ...
        let op = () => {
          ...
          if (!timedOut) {
            return this.channel.publish(message).then(onPublished, onRejected);
          }
    ...
    republish: function () {
      ...
      if (undelivered.length > 0) {
        return Promise.all(
          undelivered.map((message) => this.channel.publish(message))
        );
      }
      ...
      destroyed: {
        _onEnter: function () {
          if (this.published.count() > 0) {
            logger.warn(
              `${this.type} exchange ${this.name} - ${connection.name} was destroyed ` +
                `with ${this.published.count()} messages unconfirmed`
            );
          }
          this.handlers.forEach((handle) => {
            handle.unsubscribe();
          });
          this.channel.destroy().then(() => {
            this.emit("destroyed");
      ...
      reconnected: {
        ...
        "bindings-completed": function () {
          ...
          const onRepublishFailed = (err) => {
            logger.error(
              `Failed to republish ${this.published.count()} messages on ${
                this.type
              } exchange, ` + `${this.name} - ${connection.name}`,
              err
            );
          ...
          this.republish().then(onRepublished, onRepublishFailed);
        },
        check: function () {
          this.deferUntilTransition("ready");
        },
  ...
  exchangeMachine.on("transition", (data) => {
    logger.debug(
      `Machine exchange-${options.name}: ${data.fromState} -> ${data.toState}`
    );
  });
