 ...
 */
module.exports = function CreateQueueMachine(
  options,
  connection,
  topology,
  logger,
  ...
  const QueueMachine = machina.Fsm.extend({
    ...
    destroy: function () {
      return new Promise((resolve) => {
        log.debug(
          `Destroy called on queue ${this.name} - ${connection.name} ` +
            `(${this.channel.getMessageCount()} messages pending)`
        );
        this.on("destroyed", () => {
          this.subscriptions = [];
          resolve();
     ...
     * @param {Number} options.priority - gives a priority to the consumer; higher priority consumers get messages
          in preference to lower priority consumers. See the RabbitMQ extension's documentation
    ...
    subscribe: function (callback, options) {
      return new Promise((resolve, reject) => {
        let op = () =>
          this.channel.subscribe(callback, options).then(resolve, reject);
        ...
        _onEnter: function () {
          ...
          this.channel.destroy().then(() => {
            if (this.channel.getMessageCount() > 0) {
              log.warn(
                `!!! Queue ${options.name} - ${connection.name} was destroyed ` +
                  `with ${this.channel.getMessageCount()} pending messages !!!`
              );
            } else {
      ...
      initializing: {
        _onEnter: function () {
          ...
          this.channel.channel.once("released", () => {
            log.debug(
              `Queue ${options.name} - ${connection.name} channel released`
            );
          ...
          this.define()
            ...
            .catch((error) => {
              log.error(
                `Failed to define or resubscribe to queue ${this.name} - ${connection.name}`,
                error
              );
  ...
  queueMachine.on("transition", (data) => {
    log.debug(
      `Machine queue-${options.name}: ${data.fromState} -> ${data.toState}`
    );
  });
  return queueMachine;
};
