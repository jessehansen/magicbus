  ...
  const QueueMachine = machina.Fsm.extend({
    ...
    destroy: function () {
      return new Promise((resolve) => {
        log.debug(`Destroy called on queue ${this.name} - ${connection.name} ` +
    ...
    states: {
          ...
          this.channel.destroy()
            .then(() => {
              if (this.channel.getMessageCount() > 0) {
                log.warn(`!!! Queue ${options.name} - ${connection.name} was destroyed ` +
      ...
      initializing: {
        _onEnter: function () {
          ...
          this.define()
            ...
            .catch((error) => {
              log.error(`Failed to define or resubscribe to queue ${this.name} - ${connection.name}`, error)
      ...
      ready: {
        ...
        subscribe: function (op) {
          op()
            .then(() => {
              log.debug(`Subscription to (${options.noAck ? 'untracked' : 'tracked'}) queue ${options.name} - ${connection.name} started with consumer tag ${this.channel && this.channel.channel ? this.channel.channel.tag : ''}`)
