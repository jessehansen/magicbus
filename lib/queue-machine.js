  ...
  var Fsm = machina.Fsm.extend({
    ...
    destroy: function() {
      return new Promise(function(resolve) {
        logger.debug('Destroy called on queue %s - %s (%d messages pending)',
      ...
      'destroyed': {
        _onEnter: function() {
          if (this.channel.getMessageCount() > 0) {
            logger.warn('!!! Queue %s - %s was destroyed with %d pending messages !!!',
              options.name, connection.name, this.channel.getMessageCount());
          } else {
            logger.info('Destroyed queue %s - %s', options.name, connection.name);
          }
