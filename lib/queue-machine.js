...
module.exports = function CreateQueueMachine(options, connection, topology, logger, queueFactory) {
  ...
  const QueueMachine = machina.Fsm.extend({
     ...
     */
    define: function(stateAfterDefine) {
      const onError = (err) => {
        this.failedWith = err;
        this.transition('failed');
      };
      const onDefined = () => {
        logger.debug(`Queue ${this.name} - ${connection.name} defined - transitioning to ${stateAfterDefine}`);
        this.transition(stateAfterDefine);
      };
      logger.debug(`Queue ${this.name} - ${connection.name} define called - transitioning to ${stateAfterDefine} when finished`);
      this.channel.define()
    ...
    states: {
      ...
      'reconnecting': {
        _onEnter: function() {
          logger.info(`Reconnecting to ${options.name} - ${connection.name}`);
          this.channel = QueueFactory(options, topology, logger);
          this.channel.channel.once('released', () => {
            logger.info(`Queue ${options.name} - ${connection.name} channel released`);
            this.handle('released');
      ...
      /**
       * Reconnection complete - transitions to ready after bindings are complete and subscriptions are restarted
       * @memberOf QueueMachine.prototype.states
       */
      'reconnected': {
        _onEnter: function() {
          this.emit('defined');
          logger.info(`Reconnected to ${options.name} - ${connection.name}. Resubscribing when bindings are complete.`);
        },
        'bindings-completed': function() {
          logger.info(`Bindings completed ${options.name} - ${connection.name}. Resubscribing.`);
          const onResubscribed = () => {
            this.transition('ready');
          };
          const onResubscribeFailed = (err) => {
            logger.error(`Failed to resubscribe to queue ${this.name} - ${connection.name}`, err);
          };
          this.resubscribe()
            .then(onResubscribed, onResubscribeFailed);
        },
        check: function() {
          this.deferUntilTransition('ready');
        },
