const consumerPipelineFactory = require("../../lib/middleware")
  .ConsumerPipeline;
...
describe("ConsumerPipeline", () => {
  ...
  describe("#execute", () => {
    const successiveFunctionTestFactory = (fn) => async () => {
    ...
    const emitTestFactory = (fn) => async () => {
      let emitted = 0;
      const consumerPipeline = consumerPipelineFactory();
      consumerPipeline.use((msg, actions) => {
        actions[fn]({});
      ...
      await expect(
        consumerPipeline.prepare((eventSink) => {
          eventSink.on(fn, () => {
            emitted++;
          });
        })(message)
    ...
    for (i = 0; i < funcs.length; i++) {
      fn = funcs[i];
      it(
        "should not call successive functions when middleware calls " + fn,
        successiveFunctionTestFactory(fn)
      );
