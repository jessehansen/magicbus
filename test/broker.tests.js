...
describe("Broker really using RabbitMQ", () => {
  ...
  beforeEach(async () => {
    ...
    broker.registerRoute(
      "broker-publish",
      PublisherRoutePattern({ autoDelete: true, durable: false })
    );
    broker.registerRoute(
      "broker-subscribe",
      WorkerRoutePattern({ autoDelete: true, durable: false, exclusive: true })
    );
  ...
  describe("lifetime management", () => {
    it("should be able to shutdown many times without problems", () => {
      broker.shutdown();
      return broker.shutdown().then(() => expect(broker).toBeTruthy());
    });
  ...
  describe("with consumption defaults", () => {
    it("should be able to publish and consume messages", (done) => {
      ...
      broker.consume("broker-subscribe", handler).then(() => {
        broker.publish("broker-publish", {
          routingKey: "succeed",
          payload: Buffer.from(theMessage),
        });
      });
    });
  ...
  describe("with noBatch specified", () => {
    it("should be able to publish and consume messages", (done) => {
      ...
      broker
        .consume("broker-subscribe", handler, { noBatch: true, limit: 10 })
        .then(() => {
          broker.publish("broker-publish", {
            routingKey: "succeed",
            payload: Buffer.from(theMessage),
  ...
  describe("with noAck and noBatch specified", () => {
    it("should be able to publish and consume messages", (done) => {
      ...
      broker
        .consume("broker-subscribe", handler, {
          noAck: true,
          noBatch: true,
          limit: 10,
        })
