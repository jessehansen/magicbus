...
describe("Pub/Sub integration", () => {
  ...
  beforeEach(async () => {
    ...
    publisher = magicbus.createPublisher(broker, (cfg) =>
      cfg.useRoutePattern(
        magicbus.routePatterns.publisher({ autoDelete: true, durable: false })
      )
    );
    subscriber = magicbus.createSubscriber(broker, (cfg) =>
      cfg.useRoutePattern(
        magicbus.routePatterns.worker({
          autoDelete: true,
          durable: false,
          exclusive: true,
        })
    ...
    failedQueueSubscriber = magicbus.createSubscriber(broker, (cfg) => {
    ...
    await broker.bind(publisher.getRoute().name, subscriber.getRoute().name, {
      pattern: "#",
    });
  ...
  it("should reject message when consumption fails", (done) => {
    ...
    subscriber
      .startSubscription()
      .then(() => failedQueueSubscriber.startSubscription())
      .then(() => publisher.publish(eventName, data));
  });

  // eslint-disable-next-line jest/no-disabled-tests
  it.skip("should handle a heavy load without rejecting because of a full write buffer", () => {
    ...
    return Promise.all(
      load.map((message) =>
        publisher.publish("load-test", { message: message })
      )
    );
  });
});
