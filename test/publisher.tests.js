...
describe("Publisher", () => {
  ...
  beforeEach(() => {
    mockBroker = {
      ...
      publish: jest.fn((/* routeName, routingKey, content, options */) =>
        Promise.resolve()),
    };
    logger = Logger();
  ...
  describe("publish", () => {
    ...
    it("should be fulfilled given the broker.publish calls are fulfilled", () => {
      ...
      return expect(
        publisher.publish("something-happened")
    ...
    it("should be rejected given the broker.publish call is rejected", () => {
      ...
      return expect(publisher.publish("something-happened")).rejects.toThrow(
        "Aw, snap!"
      );
    });
