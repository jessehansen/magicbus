...
describe("Topology", () => {
  ...
  const constructTopology = () =>
    createTopology(mockConnection, logger, FakeMachine, FakeMachine);
  ...
  describe("constructor", () => {
    ...
    it("should configure bindings upon reconnect", async () => {
      ...
      expect(topology.configureBindings).toHaveBeenCalledWith(
        topology.definitions.bindings,
        true
      );
    });
  });
  ...
  describe("configureBindings", () => {
    ...
    it("can be called with an object", async () => {
      await topology.configureBindings(
        {
          "some-exchange->some-queue": {
            queue: true,
            source: "some-exchange",
            target: "some-queue",
          },
          "some-exchange->some-other-queue": {
            queue: true,
            source: "some-exchange",
            target: "some-other-queue",
          },
        },
        true
      );
